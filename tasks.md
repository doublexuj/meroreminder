...
### 6.1 완료 체크 애니메이션
- [ ] 체크 서클 → 테마색 채움 + 흰색 체크마크 (scale 0→1, 0.3s ease)
- [ ] 완료 후 0.5초 딜레이 → 리스트에서 부드럽게 슬라이드 아웃 (height collapse)

### 6.2 리스트/항목 전환 애니메이션
- [ ] 사이드바 리스트 전환 시 메인 콘텐츠 fade + slide (0.2s)
- [ ] 리마인더 추가 시 새 행 slide-down (0.25s)
- [ ] 리마인더 삭제 시 행 slide-out + collapse (0.3s)
- [ ] 리마인더 인라인 확장 시 expand 애니메이션 (0.25s ease)

### 6.3 사이드바 인터랙션
- [ ] 사용자 리스트 hover — 배경색 부드러운 전환 (0.15s)
- [ ] 스마트 리스트 카드 hover — scale(1.02) + 그림자 강화

### 6.4 빈 상태 화면
- [ ] Today 비어있음 — 체크 원 아이콘 + "All Clear for Today"
- [ ] 일반 리스트 비어있음 — 연한 회색 아이콘 + "No Reminders"
- [ ] My Lists 비어있음 — "No lists yet" 안내 텍스트

### 6.5 에러/로딩 처리
- [ ] API 호출 중 로딩 인디케이터 (스피너 또는 스켈레톤)
- [ ] API 에러 시 토스트 메시지 또는 인라인 에러
- [ ] 낙관적 업데이트 — 완료 토글, 삭제 시 즉시 UI 반영 후 API 호출

### 6.6 반응형 디자인
- [ ] Desktop (>1024px) — 사이드바 항상 표시, 고정 280px
- [ ] Tablet (768-1024px) — 사이드바 오버레이 모드, 상단 햄버거 메뉴로 토글
- [ ] Mobile (<768px) — 사이드바 풀스크린 오버레이, 리스트 선택 시 자동 닫힘
